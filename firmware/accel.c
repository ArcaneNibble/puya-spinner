...
static uint8_t i2c_read_reg;
static uint8_t i2c_read_len;
static uint8_t i2c_read_idx;
static uint8_t i2c_read_result[64];
...
void I2C1_IRQHandler() {
    ...
    if (sr1 & (1 << 6)) {
        // RxNE
        i2c_read_result[i2c_read_idx++] = I2C_DR;
        i2c_read_len--;
        if (i2c_read_len == 1) {
...
void accel_read_multi_reg(uint8_t reg, uint8_t len, uint8_t *out) {
    ...
    while (1) {
        if (i2c_cmd == CMD_FINISHED) {
            atomic_signal_fence(memory_order_acquire);
            memcpy(out, i2c_read_result, len);
            return;
        }
        asm volatile("wfi");
    }
